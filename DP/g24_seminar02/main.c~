/** 
  * Seminar Session #1: Multi-precision arithmetic

  Slow simulation: use the native Simulator provided in mcu8051ide
  Fast simulation: sdcc-s51 -X 12000000 -t 51 main.ihx < simulation.dat
**/

#include <8051.h>

#define SIZE 128

//======================================================
// Do not delete this, it is used by the fast simulator
volatile __xdata __at (0xFFFE) unsigned char startBrk;
volatile __xdata __at (0xFFFF) unsigned char endBrk;
//======================================================

// Global external RAM, specified with the __xdata parameter
//  __at (0x000) indicates the starting memory position
/*volatile __xdata __at (0x0000) unsigned char x[SIZE] = {};*/

volatile __xdata __at (0x0000) unsigned char x[SIZE] = {};  //1F


volatile __xdata __at (0x0080) unsigned char y[SIZE] = {};

volatile __xdata __at (0x0100) unsigned char m[SIZE] = {};

volatile __xdata __at (0x180) unsigned char sprime[SIZE] = 
{};


volatile __xdata __at (0x300) unsigned char resA[SIZE];
volatile __xdata __at (0x400) unsigned char resS[SIZE];
volatile __xdata __at (0x500) unsigned char resM[SIZE*2];

char c; //global signal bit
char cA;
int temp, i;
// Global internal RAM (chosen by the compiler by default)


// Functions go here
// multi-precision addition
void mp_addition(unsigned char *res, unsigned char *in1, unsigned char *in2)
{
	/**  Multi-precision addition **/
	int i;
	int temp = 0;
	
	c = 0;	
		
	for (i=0 ; i<SIZE; i++)
	{
		temp = in1[i] + in2[i] + c;
		c = temp>>8;
		//if(c==1)
		//  res[i] = (char)(temp - (1<<8));
		//else
		  res[i] = (char)(temp);
	}
        c = (char)c;
}	

// multi-precision substraction
void mp_substraction(unsigned char *res, unsigned char *in1, unsigned char *in2)
{
	/**  Multi-precision subtraction **/
	int temp = 0, i;
	c = 0;	
	
	for (i=0 ; i<SIZE; i++)
	{
		temp = in1[i] - in2[i] + c;
		c = (char)(temp>>15) & 0x0001;
		if(c==1)
			c = 0xFF;
		else 
			c = 0;
		res[i] = (char)(temp);
	}
}

// multi-precision multiplication
void mp_multiplication(unsigned char *res, unsigned char *in1, unsigned char *in2)
{
	/**  Multi-precision multiplication **/
	unsigned short c = 0, i, j;
	unsigned long temp = 0;
	
	for (i=0 ; i<SIZE*2; i++)
	{
		res[i] = 0;
	}
	for (i=0 ; i<SIZE; i++)
	{
		c = 0;
		for (j=0 ; j<SIZE; j++)
		{
			temp = res[i+j] + in1[i] * in2[j] + c;
			res[i+j] = (char)(temp & 0x000000FF);
			//c = (char)((temp >> 8) & 0x000000FF);
			c = (temp >> 8) & 0x0000FFFF;
		}
		res[i+SIZE] = (char)c;
	}
}

// copy data in external memory
void copy(unsigned char *res, unsigned char *in)
{
	short i;
	
	for (i=0 ; i<SIZE; i++)
	{
		res[i] = in[i];
	}
}

// modular addition
void md_addition(unsigned char *res, unsigned char *in1, unsigned char *in2, unsigned char *m, unsigned char *temp)
{
	char ct;
	mp_addition(res, in1, in2);
	ct=c;
		
	mp_substraction(temp, res, m);
	
	if (ct | (c==0))
		copy(res, temp);
}

// modular substraction
void md_substraction(unsigned char *res, unsigned char *in1, unsigned char *in2, unsigned char *m)
{
	char ct;
	mp_substraction(res, in1, in2);
	ct=c;

	if (ct != 0)
	{
		mp_addition(res, res, m);
	}
}

// modular multiplication
void md_multiplication(unsigned char *res, unsigned char *in1, unsigned char *in2, unsigned char *m, unsigned char *sp)
{
	unsigned short i, j, mm;
	unsigned short cl = 0;  // c local
	unsigned long temp = 0;
	
	mp_multiplication(res, in1, in2);

	//mp_multiplication(res, in1, m);
	
	for (i=0 ; i<SIZE; i++)
	{
		cl = 0;
		mm = (res[i] * sp[0]) & 0x00FF;
		for (j=0 ; j<SIZE; j++)
		{
			temp = res[i+j] + mm * m[j] + cl;
			res[i+j] = (char)(temp & 0x00FF);
			//c = (temp >> 8) & 0x00FF;
			cl = (temp >> 8) & 0x0000FFFF;
		}
		res[i+SIZE] += cl;
	}

	
	mp_substraction(res, res+SIZE, m);

	if (c != 0)
	{
		copy(res, res+SIZE);
	}
}

int main()
{
	//======================================================
	// Do not delete this, it is used by the fast simulator
	startBrk = 1;
	//======================================================

	/**  Your task starts here **/
	// multi-precision operation
	/*
	mp_addition(resA, x, y);  //0x300
	resA[SIZE] = c;
	
	mp_substraction(resS, y, x);  //0x400
	resS[SIZE] = c;
	
	mp_multiplication(resM, x, y);  //0x500
	*/

  	// modular operation
  	/*
	md_addition(resA, x, y, m, resS); //0x300

	md_substraction(resS, x, y, m);   //0x400

	md_multiplication(resM, x, y, m, sprime);  //0x500
	*/
	
	//======================================================
	// Do not delete this, it is used by the fast simulator
	endBrk=1;
	//======================================================
	return 0;
}
